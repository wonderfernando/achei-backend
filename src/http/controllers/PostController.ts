...
export class PostController {
    ...
    public store = async (req:Request, res: Response) => {
        try {
            ...
            const post = await this.postRepository.create({name: data.name,age_id:data.age_id, city_id: data.city_id,contact1: data.contact1, description: data.description,gender: data.gender,img:"",status:"",type:data.type,user_id:data.user_id,foundAt: data.foundAt, localFound: data.localFound, localDisaper: data.localDisaper,contact2: data.contact2,img2:null,disaperAt: data.disaperAt,latitude:data.latitude,longitude: data.longitude,}) 
    ...
    public update = async (req:Request, res: Response) => {
        try {
            const data = schemaPostInput.parse(req.body)
            const {id} = z.object({id:z.string()}).parse(req.params)
            const post = await this.postRepository.update(data,id) 
