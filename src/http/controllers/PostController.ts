...
import { ListAllPostSearchService } from "../../services/ListPostSearchService"
...
const  schemaPostInput = z.object({
    ...
    gender: z.string(),
    age_id:z.string(),
    disaperAt: z.date().nullable(),
    foundAt: z.date().nullable(),
...
export class PostController {
    ...
    public search = async (req:Request, res : Response) => {
        try {
        const schemQuery  = z.object({name:z.string().nullable(),age:z.string().nullable(),province:z.string().nullable()}).parse(req.query)
        
        const posts = await new ListAllPostSearchService(this.postRepository).execute(schemQuery.age,schemQuery.province,schemQuery.name)
    ...
    public store = async (req:Request, res: Response) => {
           ...
           if (!data) {
            ...
            const dataValidatr = schemaPostInput.parse(data)
            const idUser = req.id
            const img = req.file?.path || ""
            const userRepository = new UserRepositoryPrisma()
            const post = await new AddPostService(this.postRepository,userRepository).execute({name: data.name,age_id:data.age_id, city_id: data.city_id,contact1: data.contact1, description: data.description,gender: data.gender,img:img,status:"",type:data.type,user_id:idUser,foundAt: data.foundAt, localFound: data.localFound, localDisaper: data.localDisaper,contact2: data.contact2,img2:null,disaperAt: data.disaperAt,latitude:data.latitude,longitude: data.longitude}) 
